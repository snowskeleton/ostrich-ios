...
extension Gamestateschema {
  class CreateTeamMutation: GraphQLMutation {
    ...
    static let operationDocument: ApolloAPI.OperationDocument = .init(
      definition: .init(
        #"mutation createTeam($eventId: ID!, $players: [TeamPlayerInput]) { createTeam(eventId: $eventId, players: $players) { ...TeamFields __typename } }"#,
        fragments: [RegistrationDetails.self, ReservationDetails.self, TeamFields.self]
      ))

    public var eventId: ID
    public var players: GraphQLNullable<[TeamPlayerInput?]>

    public init(
      eventId: ID,
      players: GraphQLNullable<[TeamPlayerInput?]>
    ) {
    ...
    struct Data: Gamestateschema.SelectionSet {
      ...
      static var __selections: [ApolloAPI.Selection] { [
        .field("createTeam", CreateTeam.self, arguments: [
      ...
      /// Optionally include a list of players to create the team with
      /// Optionally set a flag to automatically register the team
      var createTeam: CreateTeam { __data["createTeam"] }

      /// CreateTeam
      ///
      /// Parent Type: `TeamPayload`
      struct CreateTeam: Gamestateschema.SelectionSet {
        let __data: DataDict
        init(_dataDict: DataDict) { __data = _dataDict }
